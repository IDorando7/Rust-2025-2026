use std::fmt;
use std::ops::{Add, Mul, Neg, Sub};

#[derive(Debug, Clone, Copy, PartialEq)]
struct Complex {
    real: f64,
    imag: f64,
}

impl Complex {
    fn new<R, I>(real: R, imag: I) -> Self
    where
        R: Into<f64>,
        I: Into<f64>,
    {
        Self {
            real: real.into(),
            imag: imag.into(),
        }
    }

    fn conjugate(&self) -> Self {
        Self {
            real: self.real,
            imag: -self.imag,
        }
    }
}

impl From<i32> for Complex {
    fn from(value: i32) -> Self {
        Self {
            real: value as f64,
            imag: 0.0,
        }
    }
}

impl From<f64> for Complex {
    fn from(value: f64) -> Self {
        Self {
            real: value,
            imag: 0.0,
        }
    }
}

impl<T> Add<T> for Complex
where
    T: Into<Complex>,
{
    type Output = Complex;

    fn add(self, x: T) -> Self::Output {
        let x_c: Complex = x.into();
        Complex {
            real: self.real + x_c.real,
            imag: self.imag + x_c.imag,
        }
    }
}

impl<T> Sub<T> for Complex
where
    T: Into<Complex>,
{
    type Output = Complex;

    fn sub(self, x: T) -> Self::Output {
        let x_c: Complex = x.into();
        Complex {
            real: self.real - x_c.real,
            imag: self.imag - x_c.imag,
        }
    }
}

impl<T> Mul<T> for Complex
where
    T: Into<Complex>,
{
    type Output = Complex;

    fn mul(self, x: T) -> Self::Output {
        let x_c: Complex = x.into();
        // (a+bi)*(c+di) = (ac - bd) + (ad + bc)i
        Complex {
            real: self.real * x_c.real - self.imag * x_c.imag,
            imag: self.real * x_c.imag + self.imag * x_c.real,
        }
    }
}

impl Neg for Complex {
    type Output = Complex;

    fn neg(self) -> Self::Output {
        Complex {
            real: -self.real,
            imag: -self.imag,
        }
    }
}

impl fmt::Display for Complex {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let r = self.real;
        let i = self.imag;

        let real_zero = r == 0.0;
        let imag_zero = i == 0.0;

        if real_zero && imag_zero {
            write!(f, "0")
        } else if imag_zero {
            write!(f, "{}", r)
        } else if real_zero {
            write!(f, "{}i", i)
        } else if i >= 0.0 {
            write!(f, "{}+{}i", r, i)
        } else {
            write!(f, "{}-{}i", r, -i)
        }
    }
}

fn eq_rel(x: f64, y: f64) -> bool {
    (x - y).abs() < 0.001
}
macro_rules! assert_eq_rel {
    ($x:expr, $y: expr) => {
        let x = $x as f64;
        let y = $y as f64;
        let r = eq_rel(x, y);
        assert!(r, "{} != {}", x, y);
    };
}

fn main() {
    let a = Complex::new(1.0, 2.0);
    assert_eq_rel!(a.real, 1);
    assert_eq_rel!(a.imag, 2);

    let b = Complex::new(2.0, 3);
    let c = a + b;
    assert_eq_rel!(c.real, 3);
    assert_eq_rel!(c.imag, 5);

    let d = c - a;
    assert_eq!(b, d);

    let e = (a * d).conjugate();
    assert_eq_rel!(e.imag, -7);

    let f = (a + b - d) * c;
    assert_eq!(f, Complex::new(-7, 11));

    assert_eq!(Complex::new(1, 2).to_string(), "1+2i");
    assert_eq!(Complex::new(1, -2).to_string(), "1-2i");
    assert_eq!(Complex::new(0, 5).to_string(), "5i");
    assert_eq!(Complex::new(7, 0).to_string(), "7");
    assert_eq!(Complex::new(0, 0).to_string(), "0");

    let h = Complex::new(-4, -5);
    let i = h - (h + 5) * 2.0;
    assert_eq_rel!(i.real, -6);

    let j = -i + i;
    assert_eq_rel!(j.real, 0);
    assert_eq_rel!(j.imag, 0);

    println!("ok!");
}
